//
// Assignment 1 249
// Written by: Nicole Koran (40281430) & Justin Tran (40281429)
//

/*
 * This Java program is designed to manage a library's inventory and client information. 
 * It is intended for internal use by library employees and includes functionality to handle 
 * items such as books, journals, and media, as well as client records.
 */
...
public class Driver_Library_System {
	...
	System.out.println("Welcome to the Fun Readings Library Manager! (created by Nicole and Justin)");
	...
	public static void mainMenu() {
		...
		while (more) {
			...
			scan.nextLine();
				...
				case 13: System.out.println("The biggest book is: " + getBiggestBook(makeBookArray(items))); break;
				case 14: {
				    Book[] copiedBooks = copyBooks(makeBookArray(items));
				    if (copiedBooks != null) {
				        System.out.println("Deep copy of books has been made. \nHere are the deep copies of books:");
				        for (int i = 0; i< copiedBooks.length; i++) {
				            System.out.println(copiedBooks[i].toString());
				        }
				    } else {
				        System.out.println("No books found to make a deep copy.");
				    }
				    break;
				}
	...
	public static String displayAllItems() {
		...
		if (items.length == 0) {
			System.out.println("Currently no items.");
			return null;
		}
    ...
    // case 14: make a book array and then make deep copy of it
    public static Book[] makeBookArray(Item[] items) {
    	Book[] booksArray = new Book[items.length];
    	int bookCount = 0;
    	boolean noBooks = true;
    	
    	// Iterate through all items to find books
        for (int i = 0; i < items.length; i++) {
            if (items[i] instanceof Book && items[i] != null) {
                booksArray[bookCount] =  (Book) items[i];
                bookCount++;
                noBooks = false;
                }
            }
    ...
    // make a deep copy of the books
    public static Book[] copyBooks(Book[] originalBooks) {
        if (originalBooks == null) {
            return null;
        }

        // Create a new array with the same length as the originalBooks array
        Book[] copiedBooks = new Book[originalBooks.length];

        // Iterate through the originalBooks array and create a deep copy of each book
        for (int i = 0; i < originalBooks.length; i++) {
            if (originalBooks[i] != null) {
                copiedBooks[i] = new Book(
                    originalBooks[i].getName(),
                    originalBooks[i].getAuthor(),
                    originalBooks[i].getYearOfPublication(),
                    originalBooks[i].getNumberOfPages()
                );
            }
        }
